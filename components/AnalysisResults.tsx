...
import ZoneExplorer from './ZoneExplorer'; // This file exists now!
...
const AnalysisResults: React.FC<AnalysisResultsProps> = ({ result, input }) => {
  ...
  const chartData = [
    { name: 'IST', price: currentPerSqm, color: '#334155' }, // Darker grey for current
    { name: 'MARKT', price: targetPerSqm, color: '#f5931f' } // Orange for potential
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-7 space-y-6">
           ...
           <div className="bg-slate-900/50 rounded-2xl border border-slate-800 p-6">
             <h4 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-4 border-b border-slate-800 pb-2">
               Einflussfaktoren
             </h4>
                ...
                {result.featureImpacts.map((feat, i) => (
                  <div key={i} className="flex gap-3">
                    <div className={`shrink-0 w-6 h-6 rounded flex items-center justify-center ${
                      feat.direction === 'positive' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'
                    }`}>
