...
import { UserInput, AnalysisResult, GroundingSource } from "./types"; // Correct import
...
export const analyzePotential = async (input: UserInput): Promise<AnalysisResult> => {
  ...
  try {
    const response = await ai.models.generateContent({
      model: 'gemini-2.0-flash-exp', // Updated to latest flash model for better JSON
      contents: prompt,
      config: {
        tools: [{ googleSearch: {} }],
        ...
        responseSchema: {
          ...
          properties: {
            ...
            estimatedTotalMarketRent: { type: Type.NUMBER }, // Allow AI to suggest total
    ...
    // Fallback calculations if AI returns zeros
